...
N_ALLELES      = 5       # {0: tanh, 1: sine, 2: swish, 3: relu, 4: sigmoid}
POP_SIZE       = 20      # population size (must be even)
N_GENERATIONS  = 30      # number of generations
CROSSOVER_PROB = 0.9     # probability of crossover per pair
MUTATION_PROB  = 0.15    # probability of mutating each gene
...
def run_nsga2(X_u, u_train, X_f, X_star, u_star, lb, ub,
    ...
    for gen in range(start_gen, N_GENERATIONS):
        ...
        # 1. Generate offspring chromosomes
        offspring_chromosomes = make_offspring(population, rng)

        # 2. Evaluate offspring
        offspring_results = evaluate_population(
            offspring_chromosomes, X_u, u_train, X_f, X_star, u_star,
            lb, ub, device=device,
        ...
        # 3. Combine parents + offspring (size = 2 * POP_SIZE)
...
def _print_generation(summary):
    ...
    print(f"  Gen {g:2d} | best f1={f1m:.4e} | best f2={f2m:.4e} | "
          f"pareto_size={ps} | time={t}s")